...
vec3 ambient = spotlight.Lambient * material4.Mambient;
...
vec3 diffuse = spotlight.Ldiffuse * material4.Mdiffuse * sDotN;
vec3 specular = vec3(0.f);
	if(sDotN > 0.f){
	specular = spotlight.Lspecular * material4.Mspecular * pow(max(dot(h, n), 0.f), material4.Mshininess);
	}
...
void main(){
	...
	vec3 vtColour = BPSLM(spotlight7, p, normalize(n));
