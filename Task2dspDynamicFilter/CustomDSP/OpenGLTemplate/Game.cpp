...
void Game::Initialise()
{
	...
	m_pAudio->LoadMusicStream("Resources\\Audio\\sfy.mp3"); //Song designed by The Mirages (Aleksander Napieralski).
	...
	m_Road->CreateTrack();
	m_Road->TRender("resources\\textures\\", "dirtpile01.jpg");
}
...
void Game::Render()
{
	...
	//Renders audi (Car) //Copied from Aleksander Napieralski Task2dsp
	modelViewMatrixStack.Push();
...
{
	m_t += (float)(0.1f * m_dt);
	m_currDist += m_dt * 0.1f * m_Speed; //can use m_cameraSpeed if introduced.
	//Calculates required information for CatmullRom splines for the side road.
	glm::vec3 ap;
	glm::vec3 aN;
	m_Road->Sample(m_currDist, ap);
	m_Road->Sample(m_currDist + 0.1f, aN);
	...
	m_pAudio->Update(car_position, m_pCamera->GetUpVector(), m_pCamera->GetStrafeVector(), m_pCamera->GetPosition());
	...
	//if (ifSoundEffect == true) {
	//}
	/*if (ifSoundEffect == false) {
		m_pAudio->PlayMusicStream();
	}*/
